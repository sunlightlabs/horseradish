...
from photolib.views import (ImageListView, ImageDetailView, ImageUpdateView,
	ImageUploadView, TaggedImageListView, TagListView, ImageDeleteView, ImageCropView)


urlpatterns = patterns('',
    url(r'^image/(?P<uuid>\w+)/$', login_required(ImageDetailView.as_view()), name='photolib.image_detail'),
    url(r'^image/(?P<uuid>\w+)/crop/$', login_required(ImageCropView.as_view()), name='photolib.image_crop'),
